<div class="flex justify-center">
    <div class="m-10 p-4 bg-white rounded-md text-black" style="width: 70rem">
        <div class="m-10 text-xl font-semibold">Ma promo</div>
        <div class="flex justify-center gap-20 m-10 ">
            <div class="" style="width: 80%; height: 40%">
                <img src="http://localhost/simplonImgs/imgBrief.svg" class="rounded-md" style="width: 100%; height: 100%" alt="">
            </div>
            <div class="flex flex-col justify-center gap-8">
                <div class="text-xl font-semibold">YouCode Safi 2022 Brendan Eich</div>
                <div class="text-sm">Promotion 2022 du referentiel developpement web et web mobile & CDA a la<br>fabrique de YouCode Safi au Maroc</div>
                <div class="flex gap-20">
                    <div class="flex gap-2">
                        <div><jsp:include page="../../svg/pos.jsp"/></div>
                        <div class="text-xs self-center">INT&nbsp;-&nbsp;Maroc</div>
                    </div>
                    <div class="flex gap-2">
                        <div><jsp:include page="../../svg/profiles.jsp"/></div>
                        <div class="text-xs self-center">18&nbsp;apprenants</div>
                    </div>
                    <div class="flex gap-2">
                        <div><jsp:include page="../../svg/date.jsp"/></div>
                        <div class="text-xs self-center">Du&nbsp;16/09/2022&nbsp;au&nbsp;28/03/2023</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>